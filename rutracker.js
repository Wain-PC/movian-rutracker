 ...
 (function (plugin) {
    var config = {
        ...
        regExps: {
			...
			mainSearch: /<a data-topic.*href="viewtopic\.php\?t=([\d]{0,200}?)">([\s\S]*?)<\/a>/g,
            login: /<a id="logged-in-username" class="truncated-text" href=".*u=[\d]{0,200}?">([\s\S]*?)<\/a>/g,
            mainCategoryHeader: /<h3 class="cat_title"><a href=".*?">([\s\S]*?)<\/a><\/h3>/g,
            mainSubforum: /<h4 class="forumlink"><a href="viewforum\.php\?f=([\s\S]{0,200}?)">([\s\S]*?)<\/a><\/h4>/g,
			topic: /<[a-zA-Z].*class=.*href="viewtopic\.php\?t=([\d]{0,200}?)" .*class=".*tt-text*?">([\s\S]*?)<\/a>/g,
			bookmarks: /<span.* href="viewtopic\.php\?t=([\d]{0,200}?)" .*class=".*text*?">([\s\S]*?)<\/a>/g,
            userCookie: /bb_session=\d+/,
            captcha: /<div><img src="\w+:\/\/(.*?)"[.\w\W]*?<input type="hidden" name="cap_sid" value="(.*?)">[.\w\W]*?<input class="reg-input" type="text" name="(.*?)"/g,
    ...
    settings.createBool("torrentLink", "Видимость Torrent ссылок", 0, function (v) {
    ...
    //search
    plugin.addURI(config.prefix + ":search:(.*):(.*)", function (page, forumId, search ) {
		var search,
		topicItem,
		forumId,
        tryToSearch = true,
        url = config.urls.base + config.urls.parts.search + encodeURIComponent(search)+"&f="+forumId+"&o=10",
        pageNum = 0;
		itemCount = 0;
		topicCount = 0;		
    ...
    plugin.addURI(config.prefix + ":start", function (page) {
        ...
        //while (mainSubforum) {
			...
			if (!!loginState) {
				...
				page.appendItem(config.prefix + ":logout:true:null:null", "directory", {
					title: new showtime.RichText(coloredStr("Выйти из аккаунта " + loginState[1], config.colors.red)),
					icon: logoexit
				});
			}
    ...
    plugin.addURI(config.prefix + ":forum:(.*):(.*):(.*)", function (page, forumId, forumPage, forumTitle) {
        ...
        function subforumLoader() {
            var response, dom, nextURL, textContent, imglogo,
                html = require('showtime/html');
            if (!tryToSearch) {
							  return page.haveMore(false);
            }
			...
			if (pageNum === 1){
				page.appendItem(config.prefix + ":search:"+forumId+":", 'search', {
    ...
    plugin.addURI(config.prefix + ":topic:(.*):(.*)", function (page, topicId, topicTitle) {
        ...
        function topicLoader() {
            ...
            if (pageNum === 1) {
				...
				page.appendItem("", "separator", {
					title: new showtime.RichText("Сиды: " + coloredStr(seedCount, config.colors.green) + " | Личи: " + coloredStr(lichCount, config.colors.red))
    ...
    plugin.addSearcher(plugin.getDescriptor().id, config.logo, function (page, query) {
        var url = config.urls.base + config.urls.parts.search + encodeURIComponent(query)+"&o=10",
        ...
        function loader() {
            var response, match, dom, textContent, logofilm,
                html = require('showtime/html');
            if (!tryToSearch) {
							return page.haveMore(false);
            }
            ...
            while (match && match.title !== "") {
                ...
                page.appendItem(config.prefix + ":topic:" + match.topicId + ":" + encodeURIComponent(match.title), "directory", {
